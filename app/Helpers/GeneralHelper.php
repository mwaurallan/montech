...
//use App\Http\Controllers\OpenPathSmsController;
use App\Http\Controllers\UjumbeSMSGateway;
...
//determine monthly payment using amortization
    ...
    public static function loan_total_paid($id, $date = '')
    {
        if (empty($date)) {
            ...
            return LoanTransaction::where('loan_id', $id)->where('transaction_type',
                'repayment')->where('reversed', 0)->where('date', '<=', $date)->sum('credit');
        }
...
//        $msg = rawurlencode($msg);
            // if (Setting::where('setting_key', 'sms_enabled')->first()->setting_value == 1) {
                ...
                //     'message' => $msg,
                // ];
                $sms = new UjumbeSMSGateway();
                $sms->send($to,$msg,$sender);
            // }
        }

    }

    public static function send_sms3($to, $msg, $user = null)
    {
        if ($to[0] === '0') {
            $to = '254' . ltrim($to, '0');
        }
...
//     public static function send_sms2($to, $msg)
//     {
//         if ($to[0] === '0') {
//             $to = '254' . ltrim($to, '0');
//         }
...
//                     'active_sms')->first()->setting_value);
// //                print_r($active_sms);die;
//                 $append = "&";
//                 $append .= $active_sms->to_name . "=" . $to;
//                 $append .= "&";
//                 $append .= "source=" . $active_sms->from_name;
    ...
    public static function check_outs_total_amount($start_date = '', $end_date = '')
    {
        if (empty($start_date)) {
            ...
            foreach (ProductCheckout::where('branch_id', session('branch_id'))->get() as $key) {
                $due = $due + GeneralHelper::check_out_total_amount($key->id);
            }
            return $due;
        } else {
            $due = 0;
